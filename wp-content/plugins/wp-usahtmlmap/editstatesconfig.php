...
if(isset($_POST['act_type']) && $_POST['act_type'] == 'usacustom-html5-map-states-save') {
    ...
    if ($sId) {
        ...
        if(isset($_POST['URL'][$vals['id']]))
            $states[$s_id]['link'] = usacustom_html5map_plugin_chk_url(stripslashes($_POST['URL'][$vals['id']]));
        ...
        if(isset($_POST['color'][$vals['id']]))
            $states[$s_id]['color_map'] = usacustom_html5map_plugin_chk_color($_POST['color'][$vals['id']]);

        if(isset($_POST['color_'][$vals['id']]))
            $states[$s_id]['color_map_over'] = usacustom_html5map_plugin_chk_color($_POST['color_'][$vals['id']]);
        ...
        foreach($states as $k=>$v) {
            $states[$k]['color_map'] = usacustom_html5map_plugin_chk_color($_POST['color'][$vals['id']]);
        }
        }

        if(isset($_POST['colorOverCh'][$vals['id']])) {
        foreach($states as $k=>$v) {
            $states[$k]['color_map_over'] = usacustom_html5map_plugin_chk_color($_POST['color_'][$vals['id']]);
        }
        }

    } else {

        if (count($_REQUEST['bulks'])) {
...
echo "<div class=\"wrap usacustom-html5-map main full\"><h2>" . __('Configuration of Map Areas', 'usacustom-html5-map') . "</h2>";
?>
<script>
    var map_cfg = {
...
?>
    ...
    jQuery(function($){
        ...
        jQuery('select[name=state_select]').change(function() {
            location.href='admin.php?page=usacustom-html5-map-states&map_id=<?php echo $map_id; ?>&s_id='+jQuery(this).val();
        });
...
<?php
    ...
    usacustom_html5map_plugin_map_selector('states', $map_id, $options);
    echo "<br /><br />\n";
    usacustom_html5map_plugin_nav_tabs('states', $map_id);

?>

    <p><?php echo __('This tab allows you to add the area-specific information and adjust colors of individual area on the map.', 'usacustom-html5-map'); ?></p>
    <p class="help"><?php echo __('* The term "area" means one of the following: region, state, country, province, county or district, depending on the particular plugin.', 'usacustom-html5-map'); ?></p>

    <select name="state_select" class="bulk-hide">
        <option value=0><?php echo __('Select an area', 'usacustom-html5-map'); ?></option>
        <?php
    ...
    <button type="button" class="button button-secondary map-show" style="margin-top: 1px" data-toggle="<?php esc_attr_e('Hide map', 'usacustom-html5-map'); ?>"><?php _e('Choose on map', 'usacustom-html5-map') ?></button>

    <button class="button button-secondary bulk-selectall notbulk-hide" style="margin-top: 1px"><?php _e('Toggle selections', 'usacustom-html5-map') ?></button>

    <?php if ($isbulk) { ?>
        <a href="admin.php?page=usacustom-html5-map-states&map_id=<?php echo $map_id; ?>" class="button button-secondary bulk-editing-classic" style="margin-top: 1px"><?php _e('Back to one-by-one editing', 'usacustom-html5-map') ?></a>
    <?php } else { ?>
        <a href="admin.php?page=usacustom-html5-map-states&map_id=<?php echo $map_id; ?>&bulk" class="button button-secondary bulk-editing bulk-hide" style="margin-top: 1px"><?php _e('Bulk Edit', 'usacustom-html5-map') ?></a>
    <?php } ?>
    ...
    <fieldset>
        <legend><?php echo __('Map area', 'usacustom-html5-map'); ?></legend>

        <div style="" id="stateinfo-<?php echo $vals['id']?>" class="stateinfo">
        <span class="title"><?php echo __('Name:', 'usacustom-html5-map'); ?> </span><input class="" type="text" name="name[<?php echo $vals['id']?>]" value="<?php echo $vals['name']?>" />
        <span class="tipsy-q" original-title="<?php esc_attr_e('Name of Area', 'usacustom-html5-map'); ?>">[?]</span>
        ...
        <?php echo __('do not show popup name', 'usacustom-html5-map'); ?>
        </label>
        <div class="clear"></div>

        <span class="title"><?php echo __('Shortname:', 'usacustom-html5-map'); ?> </span><input class="" type="text" name="shortname[<?php echo $vals['id']?>]" value="<?php echo $vals['shortname']?>" />
        <span class="tipsy-q" original-title="<?php esc_attr_e('Shortname of Area. Use \n to break the lines.', 'usacustom-html5-map'); ?>">[?]</span>
        <div class="clear"></div>

        <span class="title"><?php echo __('What to do when the area is clicked:', 'usacustom-html5-map'); ?></span>
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="n<?php echo $vals['id']?>" value="nill" <?php echo $rad_nill?> autocomplete="off">&nbsp;<?php echo __('Nothing', 'usacustom-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Do not react on mouse clicks', 'usacustom-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="u<?php echo $vals['id']?>" value="url" <?php echo $rad_url?> autocomplete="off">&nbsp;<?php echo __('Open a URL', 'usacustom-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('A click on this area opens a specified URL', 'usacustom-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="m<?php echo $vals['id']?>" value="more" <?php echo $rad_more?> autocomplete="off">&nbsp;<?php echo __('Show more info', 'usacustom-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Displays a side-panel with additional information (contacts, addresses etc.)', 'usacustom-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        ...
        <div style="<?php echo $style_input; ?>" id="stateURL<?php echo $vals['id']?>">
            <span class="title"><?php echo __('URL:', 'usacustom-html5-map'); ?> </span><input style="width: 240px;" class="" type="text" name="URL[<?php echo $vals['id']?>]" id="URL<?php echo $vals['id']?>" value="<?php echo $vals['link']?>" />
            <span class="tipsy-q" original-title="<?php esc_attr_e('The landing page URL', 'usacustom-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
            <label><input type="checkbox" name="isNewWindow[<?php echo $vals['id']?>]" <?php if (!empty($vals['isNewWindow'])) echo 'checked="checked" '; ?>/> <?php echo __('Open url in a new window', 'usacustom-html5-map'); ?></label></br>
        </div>
        ...
        <?php usacustom_html5map_plugin_wp_editor_for_tooltip($vals['comment'], "info[{$vals['id']}]", 'info'); ?>
        <br />
        <span class="title"><?php echo __('Area color:', 'usacustom-html5-map'); ?> </span><input class="color colorSimple" type="text" name="color[<?php echo $vals['id']?>]" value="<?php echo $vals['color_map']?>" style="background-color: #<?php echo $vals['color_map']?>"  />
        <span class="tipsy-q" original-title='<?php esc_attr_e('The color of an area.', 'usacustom-html5-map'); ?>'>[?]</span><div class="colorpicker"></div>
        <label><input name="colorSimpleCh[<?php echo $vals['id']?>]" class="colorSimpleCh" type="checkbox" /> <?php echo __('Apply to all areas', 'usacustom-html5-map'); ?></label>
        <br />
        <span class="title"><?php echo __('Area hover color:', 'usacustom-html5-map'); ?> </span><input class="color colorOver" type="text" name="color_[<?php echo $vals['id']?>]" value="<?php echo $vals['color_map_over']?>" style="background-color: #<?php echo $vals['color_map_over']?>"  />
        ...
        <label><input name="colorOverCh[<?php echo $vals['id']?>]" class="colorOverCh" type="checkbox" /> <?php echo __('Apply to all areas', 'usacustom-html5-map'); ?></label>
        <br />
        ...
        <label style="margin-left:20px; cursor: default"><span style="color: #666">ID: <?php echo 'st'.$sId ?> </span><span class="tipsy-q" original-title="<?php esc_attr_e('Use this ID when interacting with the map via API', 'usacustom-html5-map'); ?>">[?]</span></label>
        <br />

        <span class="title"><?php echo __('Hide area:', 'usacustom-html5-map'); ?> </span>
        <label><input name="hidden[<?php echo $vals['id']?>]" type="radio" value="show" <?php if (!isset($vals['hidden'])) echo 'checked="checked"'; ?> /> <?php echo __('show', 'usacustom-html5-map'); ?></label>
        <label><input name="hidden[<?php echo $vals['id']?>]" type="radio" value="hide" <?php if (isset($vals['hidden'])) echo 'checked="checked"'; ?> /> <?php echo __('hide', 'usacustom-html5-map'); ?></label>
        <br />
        </div>
    ...
    <input type="hidden" name="act_type" value="usacustom-html5-map-states-save" />
    <p class="submit"><input type="submit" value="<?php esc_attr_e('Save Changes', 'usacustom-html5-map'); ?>" class="button-primary" id="submit" name="submit" disabled="disabled"></p>
</form>
</div>
