...
function usacustom_html5map_plugin_scripts(){
    if ( is_admin() ){
        ...
        wp_register_style('usacustom-html5-map-adm', plugins_url('/static/css/mapadm.css', __FILE__), array(), 'r3.2.9');
        wp_enqueue_style('usacustom-html5-map-adm');
        wp_register_style('usacustom-html5-map-style', plugins_url('/static/css/map.css', __FILE__), array(), 'r3.2.9');
...
function usacustom_html5map_plugin_content($atts, $content) {
    ...
    wp_register_script('raphaeljs', usacustom_html5map_plugin_get_raphael_js_url(), array(), 'r3.2.9');
    wp_register_script('usacustom-html5-map-mapjs_'.$type_id, usacustom_html5map_plugin_get_map_js_url($options), array('raphaeljs'), 'r3.2.9');
    ...
    if ($options['useAjaxUrls']) {
        ...
        $popupUrl = usacustom_html5map_plugin_ajax_url('popup');
        ...
        usacustom_html5map_plugin_popup_builder_enable_scripts($options, $states);
        $popupUrl = "{$siteURL}/' + 'index.php' + '?map_id={$map_id}' + '&usacustomhtml5map_get_popup"; // splitting urls in attempt to prevent this links from beeing parsed by google
        $stateInfoUrl = "{$siteURL}/' + 'index.php' + '?map_id={$map_id}' + '&usacustomhtml5map_get_state_info="; // splitting urls in attempt to prevent this links from beeing parsed by google
        $groupInfoUrl = "{$siteURL}/' + 'index.php' + '?map_id={$map_id}' + '&usacustomhtml5map_get_group_info="; // splitting urls in attempt to prevent this links from beeing parsed by google
    }
    ...
    if ($options['areasList']) {
        ...
        wp_register_script('usacustom-html5-map-nicescroll', plugins_url($niceScrollJsFile, __FILE__), array(), '3.7.2'); // version from jquery.nicescroll.js
...
function usacustom_html5map_plugin_print_map_settings($map_id, &$map_options) {
    ...
    $mapCfg += array(
        ...
        'pointNameFontFamily'       => usacustom_html5map_plugin_escape_fonts($map_options['pointNameFontFamily'] ? $map_options['pointNameFontFamily'] : $map_options['nameFontFamily'], 'Arial, sans-serif'),
    ...
    var usacustomhtml5map_map_cfg_<?php echo $map_id ?> = <?php echo json_encode($mapCfg, $jsEncodeOption) ?>;
...
function usacustom_html5map_plugin_delete_options($blog_id = null, $option_name = 'usacustomhtml5map_options') {
    if ( is_multisite() ) {
...
function usacustom_html5map_plugin_activation() {
    ...
    if (is_multisite()) {
        add_blog_option(get_current_blog_id(), 'usacustomhtml5map_options', $options);
    } else {
...
function usacustom_html5map_plugin_uninstall() {
    if (is_multisite()) {
        if (function_exists('get_sites')) {
            $res = get_sites();
            foreach ($res as $blog) {
                delete_blog_option($blog->blog_id, 'usacustomhtml5map_options');
                delete_blog_option($blog->blog_id, 'usacustomhtml5map_goptions');
            }
        }
    } else {
        delete_site_option('usacustomhtml5map_options');
        delete_site_option('usacustomhtml5map_goptions');
    }
}
